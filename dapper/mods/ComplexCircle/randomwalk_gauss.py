...
FIG_DIR = '/home/ivo/Figures/vae/exp05'
...
def run_model(K, dko, seed, obs_type='normal', sigo=.1, sig=0., friction=0.):
    """
    Function that creates the model for this experiment
    """

    Dyn = {'M': 2, 'model': circle.uhlenbeck_factory(sig=sig,friction=friction),
           'linear': circle.uhlenbeck_factory(sig=sig,friction=friction), 'noise': 0}
...
class XpsClass:
    ...
    def __init__(self, clima, HMM, Nens, No):
        self.names = ['no DA', 'ETKF', 'single-transfer', 'single-clima', 'double-clima','double-transfer']
        self.hp = clima.hp
...
class UhlenbeckExperiment(VaeExperiment):
    """ Experiment in which truth runs over unit circle with varying radius. """

    def __init__(self, N, dko, save_name='oscillation5.pkl'):
        self.dko = dko
        ...
        self.climas = iter(ClimaExperiment())
    ...
    def run1(self, clima):
        ...
        for xp in self.xps:
            ...
            radius = np.linalg.norm(xp.stats.E.f,axis=-1)
            print('RADIUS ',np.min(radius),np.max(radius),np.mean(radius))
...
plot_movie(['ETKF', 'single-transfer',  
            'single-clima','double-clima','double-transfer'], 500, 10)
